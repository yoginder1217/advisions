...
export default function UpdateSubcategory() {
  ...
  const postData = (e) => {
    e.preventDefault();
    const item = allSubcategories.find(x => x.name && x.name.toLowerCase() === data.name.toLowerCase());
    if (item) {
  ...
  const getAPIData = useCallback(() => {
    ...
    if (allSubcategories.length) {
      const item = allSubcategories.find((x) => x.id === parseInt(id));
      if (item) {
        setData((old) => ({
          ...old,
          ...item
