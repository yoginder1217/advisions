...
export default function UpdateMaincategory() {
  ...
  const postData = (e) => {
    e.preventDefault();
    const item = allMaincategories.find(x => x.name && x.name.toLowerCase() === data.name.toLowerCase());
    if (item) {
  ...
  const getAPIData = useCallback(() => {
    ...
    if (allMaincategories.length) {
      const item = allMaincategories.find((x) => x.id === parseInt(id));
      if (item) {
        setData((old) => ({
          ...old,
          ...item
  ...
  }, [dispatch, id, allMaincategories]);

  useEffect(() => {
    getAPIData();
  }, [getAPIData]); // Now, we only include getAPIData in the dependency array
